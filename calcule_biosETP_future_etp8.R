...
ETP_8 <- function(rlist, rlist2, outfile, format='') {
  ...
  if (!file.exists(outfile)) {
    ...
    RasVals <- apply(rowVals[,3:ncol(rowVals)], 1, p8fun) ## esto se puede paralelizar
    
    p8 <- PpTaStack[[1]] #raster guia
    p8[] <- NA
    pos_with_values <- cellFromXY(object=p8, xy=rowVals[,1:2])
...
foreach(i = 1:length(models), .packages = c('raster', 'dplyr', 'gtools', 'foreach', 'stringr'), .export = 'ETP_8') %dopar% {
